...
const config = {
  nBack: 3,
  colors: ['red', 'green', 'gray', 'pink','orange'],
  // colors: ['blue', 'green'],
...
function showOpeningPage() {
  const openingText = [
    "n-Back Mario Mushroom Game!",
    "",
    `${config.nBack}-back`,
    "Rules:",
    "1. Remember the color of the mushroom that appeared",
    `   ${config.nBack} steps ago.`,
...
function generateAndDownloadCSV() {
  ...
  allLevelResults.forEach(result => {
    ...
    const filteredReactionTimes = result.reactionTimes ? result.reactionTimes.filter(time => time >= 0) : [];
    const filteredFalseReactionTimes = result.falseReactionTimes ? result.falseReactionTimes.filter(time => time >= 0) : [];

    const reactionTimesString = filteredReactionTimes.join(' ; ');
    const avgReactionTime = filteredReactionTimes.length > 0 ?
      filteredReactionTimes.reduce((a, b) => a + b, 0) / filteredReactionTimes.length : 0;
    totalTrueReactionTime += filteredReactionTimes.reduce((a, b) => a + b, 0);
    totalTrueReactionCount += filteredReactionTimes.length;

    const falseReactionTimesString = filteredFalseReactionTimes.join(' ; ');
    const avgFalseReactionTime = filteredFalseReactionTimes.length > 0 ?
      filteredFalseReactionTimes.reduce((a, b) => a + b, 0) / filteredFalseReactionTimes.length : 0;
    totalFalseReactionTime += filteredFalseReactionTimes.reduce((a, b) => a + b, 0);
    totalFalseReactionCount += filteredFalseReactionTimes.length;

    csvContent += `${result.level || ''},"${reactionTimesString}",${avgReactionTime.toFixed(4)},"${falseReactionTimesString}",${avgFalseReactionTime.toFixed(4)},${result.collected || 0}/${result.toCollect || 0},${result.avoided || 0}/${result.toAvoid || 0},${(result.collected || 0) + (result.avoided || 0)}/${(result.toCollect || 0) + (result.toAvoid || 0)},${(result.successRate || 0).toFixed(4)},${(result.avoidRate || 0).toFixed(4)},${(result.overallSuccess || 0).toFixed(4)}\n`;
 });
