  ...
  if (gameState.currentMushroom) {
    ...
    if (gameState.currentMushroom.x + config.mushroomSize < 0) {
      if ((gameState.currentMushroom.color !== gameState.targetMushroom.color)) {
        ...
        showFeedback("Success!", "#00FF00", 1000);
    
      } else{
        showFeedback("Fail!", "#FF0000", 1000);
      }
...
function drawTargetMushroom() {
  if (gameState.targetMushroom && gameState.targetShown) {
    ...
    // Draw the "Target:" text above the mushroom
    ctx.fillStyle = 'black';
    ctx.font = '20px Arial';
...
function handleLevelCompletion() {
  ...
  if (allLevelResults.length === 10) {
    ...
    }, 5000); // Wait 10 seconds before clicking the button
  }
...
function showFeedback(text, color, duration = 400) {  // Reduced from 1000 to 500 milliseconds
  gameState.feedbackText = text;
  gameState.feedbackColor = color;
...
function drawFeedback() {
  if (gameState.showFeedback) {
    ...
    const textWidth = ctx.measureText(gameState.feedbackText).width;
    ...
    ctx.fillRect(
      ...
      canvas.height / 4 - boxHeight / 2,  // Moved up to 1/4 of the screen height
      boxWidth,
      boxHeight
    );

    // Draw border
    ctx.strokeStyle = gameState.feedbackColor;
    ctx.lineWidth = 4;  // Increased from 3 to 4
    ctx.strokeRect(
      (canvas.width - boxWidth) / 2,
      canvas.height / 4 - boxHeight / 2,
      boxWidth,
      boxHeight
    );
...
function generateAndDownloadCSV() {
  ...
  allLevelResults.forEach(result => {
    ...
    totalReactionTime += result.reactionTimes.reduce((a, b) => a + b, 0);
    totalReactionCount += result.reactionTimes.length;
    // totalReactionTime += result.falseReactionTimes.reduce((a, b) => a + b, 0);
    // totalReactionCount += result.falseReactionTimes.length;
    ...
    csvContent += `${result.level},"${reactionTimesString}",${avgReactionTime.toFixed(4)},"${falseReactionTimesString}",${avgFalseReactionTime.toFixed(4)},${result.collected}/${result.toCollect}, ${result.avoided}/${result.toAvoid} ,${overallRate} ,${result.successRate.toFixed(4)},${result.avoidRate.toFixed(4)},${result.overallSuccess.toFixed(4)}\n`;
  });

  // Calculate and add overall average reaction time
